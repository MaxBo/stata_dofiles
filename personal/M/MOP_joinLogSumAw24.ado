...
************************************************************************
program define MOP_joinLogSumAw24

di "merge den Erreichbarkeitsvorteil der Personen 2-4"
quietly {

sort ID JAHR pid
merge ID JAHR using "$MOP_workingdir\Mobilitätsdaten\Long\LogSum.dta", uniqusing
drop _merge*
...
if !_rc {
	...
	replace PsonstkA = 3 if Pers24ArbeitswegkA>=3 & (C3&!P) & pid==1
}
