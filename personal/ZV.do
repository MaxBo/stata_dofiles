...
//				else if "`vm'" == "OV" {	
					...
					gen double `wij' = `scale_OV' * ( tOV_`j' + `UH' * UH_`j'+ `ZZ' * ZZ_`j'+ `SW' * SW_`j' )
					gen double `VMconst' = `const_OV'
				}
//				else if "`vm'" == "PkwMF" {	
				else if `vm' == 4 {
					gen double `wij' = `scale_Pkw' *  tPkw_`j' 
					gen double `VMconst' = `const_PkwMF'
				}
//				else if "`vm'" == "PkwF" {	
				else if `vm' == 5 {
					gen double `wij' = `scale_Pkw' *  tPkw_`j' // + `cost_Pkw' * costPkw_`j'
...
//				else if "`vm'" == "OV" {	
					...
					gen double `wij' = `scale_OV' * ( tOV_`j' + `UH' * UH_`j'+ `ZZ' * ZZ_`j'+ `SW' * SW_`j' + `const_OV')
				}
//				else if "`vm'" == "PkwMF" {	
				else if `vm' == 4 {
...
//				else if "`vm'" == "PkwF" {	
					...
					gen double `wij' = `scale_Pkw' * (tPkw_`j' + `const_PkwF') //+ `cost_Pkw' * costPkw_`j' 
...
constraint 5 [SW]_cons = 2
constraint 6 [A]_cons = -1
...
ml model lf vmziel_lf (lambda: VM = ) /t_Fuss /t_Rad /t_OV /UH /ZZ /SW (Fuss: ) (Rad: Rad_*,noconst) (OV: ) (PkwMF: Pkw_*,noconst) (PkwF: Pkw_*,noconst) /A , constraint(1/5)
