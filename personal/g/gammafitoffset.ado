...
/*------------------------------------------------ playback request */
program gammafitoffset, eclass byable(onecall)
	version 8.1
	if replay() {
		if "`e(cmd)'" != "gammafitoffset" {
			di as err "results for gammafitoffset not found"
			exit 301
		}
		if _by() error 190 
		Display `0'
...
program Estimate, eclass byable(recall)
	syntax varlist(max=1) [if] [in] [fw aw] [,  ///
		...
		Robust Cluster(varname) Level(integer $S_level) noLOG INITAlpha(real 1.) INITBeta(real 1.) * ]
...
//	qui replace `touse' = 0 if `y' < 0
	...
	`log' ml model lf gammafitoffset_lf (alpha: `alphavar') (beta: `betavar') (offset: ) ///
		`wgt' if `touse' 
	ml init `initalpha' `initbeta' `initoffset', copy
	ml maximize ,				 ///
	   `robust'       		 ///
		search(on) `clopt' `level' `mlopts' `stdopts' `modopts'

	eret local cmd "gammafitoffset"
	eret local depvar "`y'"
