...
//			OV
					...
					quietly gen double `wij' = `t_OV' * widOV_`j' 
//tOV_`j' + `UH' * UH_`j'+ `ZZ' * ZZ_`j'+ `SW' * SW_`j'
...
ml model lf zielvm_lf (t_Rad : VM = ) /t_OV /t_Pkw /const_Rad /const_OV (PkwMF: _IPkwVerf*,noconst) (PkwF: _IPkwVerf*,noconst) // /beta_e 
ml model lf zielvm_lf (t_Rad : VM = ) /t_OV /t_Pkw /const_Rad /const_OV /const_MF /const_F // /beta_e 

//(Rad: _IRadVerf*,noconst) 

ml init /t_Rad=1 /t_OV=0.5 /t_Pkw=1 /const_OV=0 PkwMF:_IPkwVerf_0=-3 // /beta_e=-0.2
