...
label define xsex 0 "männlich" 1 "weiblich" .a "keine Angabe",modify
...
// eigentlich keine räumlichen Faktoren relevant - nur Personen mit geringem Einkommen und ältere Frauen versuchen nicht mehr, den Führerschein zurück zu bekommen.
...
// Frauen geben Führerschein im Alter deutlich eher ab als Männer
// weder City noch ÖV-Erreichbarkeit noch ÖV-Startwartezeit spielen bei der Führerschein-Abgabe eine Rolle, 
// wohl aber die nahräumliche Erreichbarkeit (shop, house, ...)
// für ältere Menschen: entscheidend ist, ob Alltag in der Nähe ohne Auto organisierbar ist, dann geben sie ggf. Führerschein ab.
...
// wer noch (immer) ein Auto hat, der gibt den Führerschein nicht freiwillig ab
...
// wer noch immer ein Auto hat, der gibt den Führerschein nicht freiwillig ab
// wenn noch andere Erwachsene im Haushalt - Führerschein wird eher abgegeben (andere können mich zur Not fahren)
...
set more off
capture log close
log using "$PW_workingdir\soep\proj\FS\est\pkw\st_pkw_170709a.log",replace
...
streg _age_* _fem_* pincln adult2 adult3 kid? oa_* mixedneighb house_* MZLogSumPkw OZLogSumPkw METmittlswz shop?? , dist(e) log nohr constr(100) collin
estimates save $PW_workingdir\soep\proj\FS\est\pkw\Pkw21exp_LogSum,replace
streg _age_* _fem_* pincln mixedneighb house_* MZLogSumPkw OZLogSumPkw METmittlswz shop?? , dist(e) log nohr constr(100) collin
estimates save $PW_workingdir\soep\proj\FS\est\pkw\Pkw21exp_LogSum_ohneocc,replace
...
soep_genvar_oc_age
streg _age_* _fem_* pincln adult2 adult3 kid? oa_* mixedneighb house_* city_* shop??, dist(e) log nohr constr(100) collin
estimates save $PW_workingdir\soep\proj\FS\est\pkw\Pkw32exp_city,replace
streg _age_* _fem_* pincln adult2 adult3 kid? oa_* mixedneighb house_* MZLogSumPkw OZLogSumPkw METmittlswz shop??, dist(e) log nohr constr(100) collin
estimates save $PW_workingdir\soep\proj\FS\est\pkw\Pkw32exp_LogSum,replace
streg _age_* _fem_* pincln mixedneighb house_* MZLogSumPkw OZLogSumPkw METmittlswz shop??, dist(e) log nohr constr(100) collin
estimates save $PW_workingdir\soep\proj\FS\est\pkw\Pkw32exp_ohneocc,replace
...
soep_genvar_oc_age
streg _age_* _fem_* pincln adult2 adult3 kid? oa_* mixedneighb house_* city_* shop??  , dist(e) log nohr constr(100) collin
estimates save $PW_workingdir\soep\proj\FS\est\pkw\Pkw31exp_city,replace
streg _age_* _fem_* pincln adult2 adult3 kid? oa_* mixedneighb house_* MZLogSumPkw OZLogSumPkw METmittlswz shop??  , dist(e) log nohr constr(100) collin
estimates save $PW_workingdir\soep\proj\FS\est\pkw\Pkw31exp_LogSum,replace
streg _age_* _fem_* pincln mixedneighb house_* MZLogSumPkw OZLogSumPkw METmittlswz shop??  , dist(e) log nohr constr(100) collin
estimates save $PW_workingdir\soep\proj\FS\est\pkw\Pkw31exp_LogSum_ohneocc,replace
