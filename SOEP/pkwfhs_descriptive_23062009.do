...
set memory 800m
set more off
log using "$PW_workingdir\soep\proj\FS\pkwfhs_descriptive_23062009.log",replace
...
tab year pkw if agem>18

tab year ppkwverf if agem>18
...
stset agem ,id(persnr) failure(pkw=12) origin(time 18) enter(pkwpossible=1) exit(fhs=3 time .)
...
stphplot, by(taet) plot1opts(msymbol(i))  plot2opts(msymbol(i)) plot3opts(msymbol(i)) plot4opts(msymbol(i)) plot5opts(msymbol(i)) plot6opts(msymbol(i))
streg _age_* _fem_* pincln adult2 adult3 kid? oa_* mixedneighb house_* city_* shop?? , dist(e) log  
estimates save $PW_workingdir\soep\proj\FS\est\Pkw12exp,replace


noi di "schätze Pkw-Erwerb-Modell nie->immer"
stset agem ,id(persnr) failure(pkw=13) origin(time 18) enter(pkwpossible=1) exit(fhs=3 time .)
replace _st=0 if (pkw!=1 & pkw!=12 & pkw!=13)
stsum
soep_numfailures
...
stset agem ,id(persnr) failure(pkw=23) origin(time 18) enter(pkwpossible=1) exit(fhs=3 time .)
...
// Führerschein-Erwerb
stset agem ,id(persnr) failure(pkw=21) origin(time 18) enter(pkwpossible=1) exit(fhs=3 time .)
replace _st=0 if (pkw!=2 & pkw!=21 & pkw!=23)
stsum
soep_numfailures
stphplot, by(taet) plot1opts(msymbol(i))  plot2opts(msymbol(i)) plot3opts(msymbol(i)) plot4opts(msymbol(i)) plot5opts(msymbol(i)) plot6opts(msymbol(i))
...
stset agem ,id(persnr) failure(pkw=32) origin(time 18) enter(pkwpossible=1) exit(fhs=3 time .)
...
// Führerschein-Erwerb
stset agem ,id(persnr) failure(pkw=31) origin(time 18) enter(pkwpossible=1) exit(fhs=3 time .)
replace _st=0 if (pkw!=3 & pkw!=31 & pkw!=32)
stsum
soep_numfailures
//stphplot, by(taet) plot1opts(msymbol(i))  plot2opts(msymbol(i)) plot3opts(msymbol(i)) plot4opts(msymbol(i)) plot5opts(msymbol(i)) plot6opts(msymbol(i))
