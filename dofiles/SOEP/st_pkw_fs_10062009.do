...
set memory 800m
set more off
log using "$PW_workingdir\soep\proj\FS\st_fhs_10062009.log",replace
...
set more off
capture log close
log using "$PW_workingdir\soep\proj\FS\st_pkw_fs100609.log",replace
...
recode dist_?z (0=.5)
dropvars dist_?zln
gen dist_ozln = ln(dist_oz)
gen dist_mzln = ln(dist_mz)

dropvars s h ch
stcox female pincln adult2 adult3  oc_* mixedneighb house_* shop??  dist_oz if year>=1993,  nohr basehc(h) basesurv(s) basechazard(ch)
dropvars s h ch
...
streg _age_* _fem_* pincln adult2 adult3 oc* mixedneighb house_* city_* shop?? , dist(e) log nocons nohr vce(robust)
...
// Wer noch immer einen Pkw hat, der gibt seinen Lappen sehr selten ab. bei zeitweise Pkw ist das nicht signifikant

streg _age_* _fem_* pincln adult2 adult3 oa_* mixedneighb house_* city_* shop?? pkwzeitw pkwimmer , dist(e) log nocons nohr
...
dropvars s h ch
stcox female pincln adult2 adult3  oc_* mixedneighb house_* shop??  dist_oz dist_mz if year>=1993,  nohr basehc(h) basesurv(s) basechazard(ch)
